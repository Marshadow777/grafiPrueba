...
a, b = 75, 75  # Reducir los valores de a y b para que la curva se ajuste mejor
...
# Parámetros de la curva de Limacon
c, d = 250, 250  # Reducir los valores de a y b para que la curva se ajuste mejor
k2 = 1.75# Constante de multiplicación del ángulo
theta_increment2 = 0.05  # Incremento del ángulo
max_theta = 2 * np.pi  # Un ciclo completo

# Parámetros de la curva de Limacon
e, f = 125, 125  # Reducir los valores de a y b para que la curva se ajuste mejor
k3 = 0.9# Constante de multiplicación del ángulo
theta_increment3 = 0.02  # Incremento del ángulo
max_theta = 2 * np.pi  # Un ciclo completo
...
while True:  # Bucle infinito
    ...
    for t in np.arange(0, theta, theta_increment2):
        ...
        cv2.circle(img, (x-2, y-2), 4, (0, 255, 255), -1)  # Color rojo

    for t in np.arange(0, theta, theta_increment3):
        # Calcular las coordenadas paramétricas (x, y) para la curva de Limacon
        r = e + f * np.cos(k3 * t)
        x = int(center_x + r * np.cos(t))
        y = int(center_y + r * np.sin(t))
        ...
        # Dibujar un círculo en la posición calculada
        #cv2.circle(img, (x, y), 3, (0, 234, 0), -1)  # Color rojo
        cv2.circle(img, (x-2, y-2), 3, (0, 165, 255), -1)  # Color rojo
    ...
    for t in np.arange(0, theta, theta_increment):
        ...
        # Dibujar un círculo en la posición calculada
        #cv2.circle(img, (x, y), 3, (0, 234, 0), -1)  # Color rojo
        cv2.circle(img, (x-2, y-2), 5, (33, 67, 101), -1)  # Color rojo
    ...
    if cv2.waitKey(1) & 0xFF == 27:  # Esperar 30ms, salir con 'ESC'
        break
